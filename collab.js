...
function isAdmin(member) {
  ...
  return COLLAB_CONFIG.adminRoleId ? member.roles.cache.has(COLLAB_CONFIG.adminRoleId) : false;
}
function parseParts(str) {
  return str.split(",").map(s=>s.trim()).filter(Boolean).map(name=>({name,userId:null,userTag:null}));
}

// ── Find collab by channel ────────────────────────────────────────────────
function findCollabByChannel(channelId) {
  for (const [,c] of collabs) if (c.channelId === channelId && c.status === "approved") return c;
  return null;
}
...
function buildPartButtons(c) {
  const rows = [];
  const btns = c.parts.slice(0,25).map((p,i)=>({type:2,style:p.userId?2:1,label:p.name.slice(0,80),custom_id:`collab_take_${c.id}_${i}`,disabled:!!p.userId}));
...
function buildEditor(s) {
  ...
  return [
    ...
    {type:1,components:[
      {type:2,style:1,label:"Name & Song",   custom_id:"collab_edit_basics"},
      {type:2,style:1,label:"Description",   custom_id:"collab_edit_desc"},
      {type:2,style:1,label:"Details",       custom_id:"collab_edit_details"},
    ]},
    {type:1,components:[
      {type:2,style:1,label:"Parts",custom_id:"collab_edit_parts"},
      {type:2,style:s.ownChannel?3:2,label:s.ownChannel?"Own Channel (ON)":"Shared Channel",custom_id:"collab_toggle_channel"},
    ]},
    {type:1,components:[
...
// Management UI (host in dedicated channel)
function buildManageUI(c) {
  const partLines = c.parts.map((p,i)=>`**${i+1}.** ${p.name} — ${p.userId?`<@${p.userId}>`:"*(open)*"}`).join("\n");
  return [
    {type:17,components:[
      {type:10,content:`## Managing: ${c.name}`},
      {type:14,divider:true,spacing:1},
      {type:10,content:partLines||"*(no parts)*"},
      {type:14,divider:true,spacing:1},
      {type:10,content:"-# Use the buttons below to reset a part or close the collab."},
    ]},
    {type:1,components:[
      {type:2,style:1,label:"Reset a Part",  custom_id:`collab_manage_${c.id}`},
      {type:2,style:4,label:"Close Collab",  custom_id:`collab_close_${c.id}`},
    ]},
  ];
}
...
// Modals (no example placeholders, labels ≤ 45 chars)
const MODALS = {
  basics: () => new ModalBuilder().setCustomId("collab_modal_basics").setTitle("Name & Song")
    .addComponents(
  ...
  desc: () => new ModalBuilder().setCustomId("collab_modal_desc").setTitle("Description")
  ...
  details: () => new ModalBuilder().setCustomId("collab_modal_details").setTitle("Details")
    .addComponents(
  ...
  parts: () => new ModalBuilder().setCustomId("collab_modal_parts").setTitle("Parts")
  ...
  manage: (c) => new ModalBuilder().setCustomId(`collab_modal_manage_${c.id}`).setTitle("Reset a Part")
...
async function handleCommand(interaction) {
  ...
  // If in a collab's dedicated channel and user is the host → show manage UI
  const existing = findCollabByChannel(interaction.channelId);
  if (existing && interaction.user.id === existing.hostId) {
    L.info("COLLAB","Host using /collab in own collab channel — showing manage UI",existing.id);
    await interaction.reply({ components: buildManageUI(existing), flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral });
    return;
  }
...
async function handleButton(interaction) {
  ...
  if (cid === "collab_edit_basics")  { const s=sessions.get(interaction.user.id); if(!s) return expired(interaction); await interaction.showModal(MODALS.basics()); return; }
  if (cid === "collab_edit_desc")    { const s=sessions.get(interaction.user.id); if(!s) return expired(interaction); await interaction.showModal(MODALS.desc());   return; }
  if (cid === "collab_edit_details") { const s=sessions.get(interaction.user.id); if(!s) return expired(interaction); await interaction.showModal(MODALS.details()); return; }
  if (cid === "collab_edit_parts")   { const s=sessions.get(interaction.user.id); if(!s) return expired(interaction); await interaction.showModal(MODALS.parts());   return; }
  ...
  if (cid === "collab_toggle_channel") {
    ...
    await interaction.deferUpdate(); await interaction.editReply({components:buildEditor(s),flags:MessageFlags.IsComponentsV2}); return;
  }
  ...
  if (cid === "collab_submit") {
    ...
    const c={id:mkId(),hostId:interaction.user.id,hostTag:interaction.user.tag,guildId:interaction.guildId,...s,parts:[...s.parts],status:"pending",roleId:null,channelId:null,pendingMsgId:null,mainMsgId:null};
    ...
    if(aChan){try{const msg=await aChan.send({components:buildApprovalMsg(c),flags:MessageFlags.IsComponentsV2});c.pendingMsgId=msg.id;L.info("COLLAB","Approval sent",msg.id);}catch(e){L.error("COLLAB","Approval send failed",e.message);}}
    else{L.warn("COLLAB","Approval channel not found",COLLAB_CONFIG.approvalChannelId);}
  ...
  if (cid.startsWith("collab_approve_")||cid.startsWith("collab_deny_")) {
    ...
    const collabId = cid.replace("collab_approve_","").replace("collab_deny_","");
    ...
    try{const role=await guild.roles.create({name:`collab: ${c.name}`.slice(0,100),color:Math.floor(Math.random()*0xFFFFFF),mentionable:true});c.roleId=role.id;L.info("COLLAB","Role created",role.id);}catch(e){L.error("COLLAB","Role create failed",e.message);}
    if(c.roleId){try{const hm=await guild.members.fetch(c.hostId);await hm.roles.add(c.roleId);}catch(e){L.warn("COLLAB","Give role to host failed",e.message);}}
    let tChan;
    if(c.ownChannel){
      try{
        const opts={name:c.name.toLowerCase().replace(/[^a-z0-9]/g,"-").slice(0,100),type:ChannelType.GuildText,topic:`${c.name} | Host: ${c.hostTag}`,
    ...
    try{const msg=await tChan.send({components:[...buildCollabInfo(c),...buildPartButtons(c)],flags:MessageFlags.IsComponentsV2});c.mainMsgId=msg.id;}catch(e){L.error("COLLAB","Post failed",e.message);}
  ...
  if (cid.startsWith("collab_take_")) {
    const rest=cid.replace("collab_take_",""); const li=rest.lastIndexOf("_");
    const collabId=rest.slice(0,li); const idx=parseInt(rest.slice(li+1),10);
    const c=collabs.get(collabId);
    if(!c){await interaction.reply({components:[{type:10,content:"❌ Collab not found."}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});return;}
  ...
  if (cid.startsWith("collab_manage_")) {
    ...
    await interaction.showModal(MODALS.manage(c)); return;
  }
  ...
  if (cid.startsWith("collab_close_")) {
    ...
    if(interaction.user.id!==c.hostId&&!isAdmin(interaction.member)){await interaction.reply({components:[{type:10,content:"❌ Host or admin only."}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});return;}
    c.status="closed";
    await interaction.update({components:[{type:17,components:[{type:10,content:`Collab **${c.name}** has been closed.`}]}],flags:MessageFlags.IsComponentsV2}); return;
  }
}
...
async function handleModal(interaction) {
  ...
  if (cid.startsWith("collab_modal_manage_")) {
    ...
    if(isNaN(idx)||idx<0||idx>=c.parts.length){await interaction.reply({components:[{type:10,content:`❌ Invalid. Range: 1-${c.parts.length}.`}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});return;}
