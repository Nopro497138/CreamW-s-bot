...
# ── Dimensions ─────────────────────────────────────────────────────────────
CW, CH   = 70, 70          # cell width / height
COLS, ROWS = 9, 5
SB       = 52              # left sidebar (lane labels)
TOP      = 52              # top UI bar
BOT      = 44              # bottom status bar
IW       = SB + COLS*CW + 12   # 696
IH       = TOP + ROWS*CH + BOT # 446
GX, GY   = SB, TOP             # grid origin
...
# ── Plant catalogue ────────────────────────────────────────────────────────
PLANT_INFO = {
    "peashooter":    dict(name="Peashooter",     cost=100, col=(50,180,50)),
    "sunflower":     dict(name="Sunflower",      cost=50,  col=(255,215,0)),
    "wallnut":       dict(name="Wall-nut",       cost=50,  col=(180,130,60)),
    "cherrybomb":    dict(name="Cherry Bomb",    cost=150, col=(220,30,30)),
    "potatomine":    dict(name="Potato Mine",    cost=25,  col=(160,120,60)),
    "snowpea":       dict(name="Snow Pea",       cost=175, col=(100,200,230)),
    "chomper":       dict(name="Chomper",        cost=150, col=(170,60,200)),
    "repeater":      dict(name="Repeater",       cost=200, col=(30,160,40)),
    "puffshroom":    dict(name="Puff-shroom",    cost=0,   col=(80,120,210)),
    "sunshroom":     dict(name="Sun-shroom",     cost=25,  col=(230,190,10)),
    "fumeshroom":    dict(name="Fume-shroom",    cost=75,  col=(120,80,40)),
    "scaredy":       dict(name="Scaredy-shroom", cost=25,  col=(220,90,150)),
    "iceshroom":     dict(name="Ice-shroom",     cost=75,  col=(140,220,255)),
    "doomshroom":    dict(name="Doom-shroom",    cost=125, col=(60,20,80)),
    "squash":        dict(name="Squash",         cost=50,  col=(60,180,60)),
    "threepeater":   dict(name="Threepeater",    cost=325, col=(20,200,40)),
    "torchwood":     dict(name="Torchwood",      cost=175, col=(180,80,20)),
    "twinsunflower": dict(name="Twin Sunflower", cost=150, col=(255,195,0)),
    "tallnut":       dict(name="Tall-nut",       cost=125, col=(160,110,50)),
    "cactus":        dict(name="Cactus",         cost=125, col=(50,160,50)),
}

ZOMBIE_INFO = {
    "regular":    dict(name="Zombie",         col=(190,200,170), hcol=(200,215,185)),
    "cone":       dict(name="Conehead",        col=(200,190,160), hcol=(220,130,30)),
    "bucket":     dict(name="Buckethead",      col=(185,185,200), hcol=(160,165,175)),
    "flag":       dict(name="Flag Zombie",     col=(190,200,170), hcol=(200,215,185)),
    "polevault":  dict(name="Pole Vaulting",   col=(180,180,160), hcol=(200,200,180)),
    "newspaper":  dict(name="Newspaper",       col=(200,195,175), hcol=(230,220,190)),
    "football":   dict(name="Football",        col=(160,120,70),  hcol=(80,60,40)),
    "dancing":    dict(name="Dancing",         col=(160,80,160),  hcol=(200,100,200)),
    "balloon":    dict(name="Balloon",         col=(190,200,170), hcol=(255,80,80)),
    "digger":     dict(name="Digger",          col=(150,130,110), hcol=(100,80,60)),
    "gargantuar": dict(name="Gargantuar",      col=(100,90,70),   hcol=(80,70,55)),
    "imp":        dict(name="Imp",             col=(150,110,90),  hcol=(180,130,100)),
}
...
def draw_peashooter(draw, cx, cy):
    c = PLANT_INFO["peashooter"]["col"]
    dc = (max(0,c[0]-40), max(0,c[1]-40), max(0,c[2]-40))
    rect(draw, cx, cy+12, 10, 30, dc)            # stem
    ellipse(draw, cx, cy-4, 22, 22, c, dc, 2)    # head
    rect(draw, cx+22, cy-4, 14, 8, dc)           # barrel
    ellipse(draw, cx+6, cy-10, 5, 5, (20,20,20)) # eye
...
def draw_sunflower(draw, cx, cy):
    ...
    ellipse(draw, cx, cy-8, 14, 14, (200,130,30))  # center
    rect(draw, cx, cy+16, 8, 28, (50,160,30))       # stem
    ellipse(draw, cx+8, cy+6, 8, 5, (50,160,30))   # leaf

def draw_wallnut(draw, cx, cy):
    ellipse(draw, cx, cy, 26, 28, (180,130,60), (120,80,30), 2)
    for i in range(3):
        draw.arc([cx-22+i*7, cy-22, cx-10+i*7, cy+22],
                 start=30, end=150, fill=(120,80,30), width=2)
    ellipse(draw, cx-8, cy-6, 4, 4, (30,20,10))  # eye L
    ellipse(draw, cx+8, cy-6, 4, 4, (30,20,10))  # eye R
...
def draw_cherrybomb(draw, cx, cy):
    ...
    ellipse(draw, cx-12, cy+2, 5, 5, (255,80,80))  # shine
    ellipse(draw, cx+12, cy+2, 5, 5, (255,80,80))

def draw_potatomine(draw, cx, cy):
    ellipse(draw, cx, cy+10, 22, 14, (160,120,60), (100,70,30), 2)
    rect(draw, cx, cy-10, 6, 16, (100,70,30))  # fuse
    ellipse(draw, cx, cy-18, 4, 4, (200,50,50))  # tip
    ellipse(draw, cx-8, cy+8, 4, 4, (30,20,10)) # eye L
    ellipse(draw, cx+8, cy+8, 4, 4, (30,20,10)) # eye R

def draw_snowpea(draw, cx, cy):
    c = PLANT_INFO["snowpea"]["col"]
    dc = (60,150,180)
    rect(draw, cx, cy+12, 10, 30, (50,160,50))
    ellipse(draw, cx, cy-4, 22, 22, c, dc, 2)
    rect(draw, cx+22, cy-4, 14, 8, dc)
...
def draw_chomper(draw, cx, cy):
    ...
    # open jaw
    draw.polygon([(cx-20,cy-18),(cx+20,cy-18),(cx+20,cy+4),(cx-20,cy+4)],
                 fill=(200,60,220), outline=(130,30,150))
    draw.polygon([(cx-20,cy+4),(cx+20,cy+4),(cx+20,cy+16),(cx-20,cy+16)],
                 fill=(150,40,180), outline=(100,20,120))
    for i in range(4):   # teeth top
        tx = cx-15+i*10
        draw.polygon([(tx,cy-18),(tx+6,cy-18),(tx+3,cy-8)], fill=(255,255,255))
    for i in range(3):   # teeth bottom
        tx = cx-10+i*10
        draw.polygon([(tx,cy+4),(tx+6,cy+4),(tx+3,cy+14)], fill=(255,255,255))
...
def draw_repeater(draw, cx, cy):
    c = PLANT_INFO["repeater"]["col"]
    dc = (20,110,30)
    rect(draw, cx, cy+12, 10, 30, dc)
    ellipse(draw, cx, cy-4, 22, 22, c, dc, 2)
    rect(draw, cx+22, cy-9, 14, 6, dc)   # barrel top
    rect(draw, cx+22, cy+1, 14, 6, dc)   # barrel bottom
    ellipse(draw, cx+6, cy-10, 5, 5, (20,20,20))
...
def draw_mushroom_base(draw, cx, cy, cap_col, stem_col=(230,230,220), cap_rx=20, cap_ry=16):
    ...
    draw.chord([cx-cap_rx,cy-cap_ry*2,cx+cap_rx,cy+2], 180, 360, fill=cap_col,
               outline=(max(0,cap_col[0]-40),max(0,cap_col[1]-40),max(0,cap_col[2]-40)))

def draw_puffshroom(draw, cx, cy):
    draw_mushroom_base(draw, cx, cy, PLANT_INFO["puffshroom"]["col"])
    for dx,dy,r in [(-8,-14,5),(8,-14,5),(0,-20,4)]:
        ellipse(draw, cx+dx, cy+dy, r, r, (255,255,255,180))

def draw_sunshroom(draw, cx, cy):
    draw_mushroom_base(draw, cx, cy, PLANT_INFO["sunshroom"]["col"])
    ellipse(draw, cx, cy-8, 6, 6, (255,240,100))

def draw_fumeshroom(draw, cx, cy):
    draw_mushroom_base(draw, cx, cy, PLANT_INFO["fumeshroom"]["col"])
    for i,dx in enumerate([-12,0,12]):
        ellipse(draw, cx+dx, cy-20-i*4, 8, 8, (180,220,100,160))

def draw_scaredy(draw, cx, cy):
    draw_mushroom_base(draw, cx, cy, PLANT_INFO["scaredy"]["col"], cap_rx=16, cap_ry=24)
    ellipse(draw, cx-6, cy-2, 5, 6, (255,255,255))
    ellipse(draw, cx+6, cy-2, 5, 6, (255,255,255))
    ellipse(draw, cx-6, cy-2, 2, 3, (30,30,30))
    ellipse(draw, cx+6, cy-2, 2, 3, (30,30,30))
    draw.arc([cx-8,cy+8,cx+8,cy+18],0,180,fill=(30,20,10),width=2) # worried mouth

def draw_iceshroom(draw, cx, cy):
    draw_mushroom_base(draw, cx, cy, PLANT_INFO["iceshroom"]["col"])
...
def draw_doomshroom(draw, cx, cy):
    draw_mushroom_base(draw, cx, cy, PLANT_INFO["doomshroom"]["col"], cap_rx=26, cap_ry=20)
    ellipse(draw, cx, cy-14, 7, 7, (200,200,200))  # skull
    ellipse(draw, cx-4,cy-16,3,3,(30,20,30)); ellipse(draw,cx+4,cy-16,3,3,(30,20,30))
    draw.line([cx-5,cy-10,cx+5,cy-10],fill=(30,20,30),width=2)

def draw_squash(draw, cx, cy):
    c = PLANT_INFO["squash"]["col"]
    draw.rounded_rectangle([cx-22,cy-16,cx+22,cy+22], radius=10, fill=c,
                            outline=(30,130,30), width=2)
    for x0 in range(cx-18, cx+20, 8):
        draw.line([x0,cy-16,x0,cy+22], fill=(30,120,30), width=2)
    ellipse(draw,cx-8,cy-4,5,5,(20,20,20)); ellipse(draw,cx+8,cy-4,5,5,(20,20,20))
    draw.arc([cx-8,cy+6,cx+8,cy+16],0,180,fill=(20,20,20),width=2)

def draw_threepeater(draw, cx, cy):
    c = PLANT_INFO["threepeater"]["col"]
    dc = (10,120,20)
    draw.line([cx,cy+20,cx,cy-18],fill=dc,width=8)    # main stem
    draw.line([cx-2,cy-8,cx-24,cy-22],fill=dc,width=6)
    draw.line([cx-2,cy-8,cx+24,cy-22],fill=dc,width=6)
    for hx,hy in [(cx,cy-28),(cx-24,cy-32),(cx+24,cy-32)]:
        ellipse(draw, hx, hy, 14, 14, c, dc, 2)
...
def draw_torchwood(draw, cx, cy):
    ...
    for dx,col in [(-8,(255,80,0)),(0,(255,140,0)),(8,(255,80,0))]:
        draw.polygon([(cx+dx-6,cy-4),(cx+dx+6,cy-4),(cx+dx,cy-24)], fill=col)
...
def draw_tallnut(draw, cx, cy):
    ...
    rect(draw, cx, cy-28, 30, 10, (140,100,40))   # top crown
    for i in range(3):
        draw.arc([cx-14+i*6,cy-22,cx-6+i*6,cy+20],30,150,fill=(120,80,30),width=2)
    ellipse(draw,cx-6,cy-4,4,4,(30,20,10)); ellipse(draw,cx+6,cy-4,4,4,(30,20,10))
    draw.arc([cx-6,cy+6,cx+6,cy+16],0,180,fill=(30,20,10),width=2)

def draw_cactus(draw, cx, cy):
    c = PLANT_INFO["cactus"]["col"]
    dc = (30,120,30)
    rect(draw, cx, cy+4, 16, 40, c, dc, 2)
    rect(draw, cx-22, cy-8, 12, 6, c, dc, 2)  # left arm
    rect(draw, cx-22, cy-18, 6, 20, c, dc, 2)
    rect(draw, cx+22, cy-4, 12, 6, c, dc, 2)  # right arm
    rect(draw, cx+22, cy-20, 6, 24, c, dc, 2)
    for dy in range(-20,22,7):  # spikes
...
def draw_zombie_body(draw, cx, cy, col, hcol, scale=1.0):
    ...
    # head
    ellipse(draw, cx, cy-int(12*s), hw, int(16*s), hcol, (col[0]-30,col[1]-30,col[2]-30), int(1.5*s))
    # eyes (hollow)
    ew = int(4*s)
    ...
    # tie
    draw.polygon([(cx, cy+int(2*s)), (cx-int(4*s),cy+int(10*s)), (cx, cy+int(20*s)),
                  (cx+int(4*s),cy+int(10*s))], fill=(180,30,30))

def draw_regular_zombie(draw, cx, cy):
    zi = ZOMBIE_INFO["regular"]
    draw_zombie_body(draw, cx, cy, zi["col"], zi["hcol"])

def draw_cone_zombie(draw, cx, cy):
    zi = ZOMBIE_INFO["cone"]
    draw_zombie_body(draw, cx, cy, zi["col"], zi["hcol"])
    draw.polygon([(cx-14,cy-22),(cx+14,cy-22),(cx,cy-44)], fill=(230,120,20),(180,80,10))
    draw.polygon([(cx-14,cy-22),(cx+14,cy-22),(cx,cy-44)], outline=(180,80,10), fill=None)

def draw_bucket_zombie(draw, cx, cy):
    zi = ZOMBIE_INFO["bucket"]
    draw_zombie_body(draw, cx, cy, zi["col"], zi["hcol"])
...
def draw_football_zombie(draw, cx, cy):
    zi = ZOMBIE_INFO["football"]
    draw_zombie_body(draw, cx, cy, zi["col"], zi["hcol"], scale=1.2)
...
def draw_digger_zombie(draw, cx, cy):
    zi = ZOMBIE_INFO["digger"]
    draw_zombie_body(draw, cx, cy, zi["col"], zi["hcol"])
    rect(draw, cx, cy-28, 22, 10, (80,70,60), (60,50,40))  # helmet
    draw.line([cx+16,cy-16,cx+32,cy+8], fill=(140,130,110), width=4)  # pickaxe handle
    draw.polygon([(cx+28,cy+4),(cx+36,cy),(cx+34,cy+12)], fill=(180,180,190))

def draw_gargantuar(draw, cx, cy):
    zi = ZOMBIE_INFO["gargantuar"]
    s = 1.8
    draw_zombie_body(draw, cx, cy, zi["col"], zi["hcol"], scale=s)
    # telephone pole
...
def draw_imp(draw, cx, cy):
    zi = ZOMBIE_INFO["imp"]
    ellipse(draw, cx, cy-4, 16, 18, zi["hcol"], (100,70,50))
    rect(draw, cx, cy+12, 12, 16, zi["col"])
    ellipse(draw, cx-6,cy-12,3,3,(30,20,10)); ellipse(draw,cx+6,cy-12,3,3,(30,20,10))
    draw.polygon([(cx-16,cy-22),(cx-8,cy-18),(cx-12,cy-10)], fill=(180,30,30))  # wing L
    draw.polygon([(cx+16,cy-22),(cx+8,cy-18),(cx+12,cy-10)], fill=(180,30,30))  # wing R
...
def draw_ui(draw, state, atmo_key):
    ...
    txt(draw, f"☀️ {state.get('sun',50)}", 8, 6, f_big, (255,240,100))
    ...
    for r in range(ROWS):
        cy = GY + r*CH + CH//2
        txt(draw, f"{'🌊' if atmo_key=='pool' and r in [2,3] else str(r+1)}", 6, cy-10, font(13), (255,255,200))
    # selected plant hint
    sel = state.get("selectedPlant")
    draw.rectangle([0,GY+ROWS*CH,IW,IH], fill=a["ui"])
    if sel and sel in PLANT_INFO:
        pi = PLANT_INFO[sel]
        txt(draw, f"Selected: {pi['name']}  ☀️{pi['cost']}  →  click a Lane button to place",
...
def draw_level_select(draw, state):
    ...
    for cx,cy,cw,ch in clouds:
        ellipse(draw, cx, cy, cw, ch, (255,255,255,220))
    ...
    txt(draw, "⚔️  SELECT YOUR LEVEL", IW//2, 20, f_title, (255,255,255), anchor="mt")
    unlocked = state.get("unlockedLevels", [0])
    lvl_defs = [
        dict(name="Day 1",   atmo="day",   x=80,  y=200, waves=3, desc="A sunny afternoon. Easy zombies."),
        dict(name="Night 2", atmo="night", x=200, y=280, waves=4, desc="The moon is rising. Watch out!"),
        dict(name="Pool 3",  atmo="pool",  x=350, y=200, waves=5, desc="Zombies come from the pool too."),
        dict(name="Fog 4",   atmo="fog",   x=500, y=280, waves=5, desc="Can't see far. Stay alert."),
        dict(name="Roof 5",  atmo="roof",  x=620, y=200, waves=6, desc="Final stand. Hardest yet."),
    ]
    atmo_cols = {"day":(255,220,80),"night":(100,100,200),"pool":(80,180,220),
                 "fog":(160,160,160),"roof":(220,140,60)}
    for i,ld in enumerate(lvl_defs):
        locked = i not in unlocked
        ac = atmo_cols.get(ld["atmo"],(200,200,200))
        bc = (100,100,100) if locked else ac
        # platform
        draw.ellipse([ld["x"]-40,ld["y"]-20,ld["x"]+40,ld["y"]+20], fill=bc,
                     outline=(50,50,50) if locked else (255,255,255), width=2)
        if locked:
            txt(draw, "🔒", ld["x"], ld["y"]-8, font(20), (180,180,180), anchor="mt")
        else:
        ...
        if i < len(lvl_defs)-1:
            nd = lvl_defs[i+1]
            draw.line([ld["x"],ld["y"],nd["x"],nd["y"]], fill=(200,200,180,150), width=3)
    txt(draw, "Choose a level to begin your adventure!", IW//2, IH-25, f_sm,
        (255,255,255), anchor="mt")

# ── Tutorial Overlay ───────────────────────────────────────────────────────
def draw_tutorial_overlay(draw, step):
    draw.rectangle([0,0,IW,IH], fill=(0,0,0,160))
    tips = [
        ("☀️  Collecting Sun", 150,
         "Sun is your currency.\nSunflowers produce it.\nCollect enough to buy plants!"),
        ("🌱  Planting", 200,
         "Select a plant below,\nthen click a Lane button\nto plant it in that row."),
        ("🧟  Zombies!", 250,
         "Zombies walk from right\nto left. Don't let them\nreach your house!"),
        ("⚡  Taking Turns", 300,
         "Press ⚡ Next Turn to\nadvance time. Plants shoot\nand zombies move each turn."),
        ("🏪  Shop", 350,
         "Between waves, open the\nShop to unlock new plants\nusing your sun."),
    ]
    if step < len(tips):
        label, hy, desc = tips[step]
        f_h = font(22, bold=True); f_d = font(15)
        # spotlight box
        draw.rounded_rectangle([IW//2-240, hy-60, IW//2+240, hy+100],
                                radius=16, fill=(40,40,60,230), outline=(255,220,80), width=3)
        txt(draw, label, IW//2, hy-44, f_h, (255,220,80), anchor="mt")
        for i,line in enumerate(desc.split("\n")):
            txt(draw, line, IW//2, hy-14+i*22, f_d, (220,230,255), anchor="mt")
...
def draw_endscreen(draw, won):
    ...
    txt(draw, "Press Restart to play again.", IW//2, IH//2+45, f_sm, (200,200,200), anchor="mt")
...
def render(state, out_path):
    ...
    # ── Projectiles ─────────────────────────────────────────────────
    proj_cols = {"pea":(80,200,80),"snow":(150,230,255),"fire":(255,120,20),
                 "spike":(200,200,80),"fume":(150,220,100,160)}
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: pvz_render.py <output.png>", file=sys.stderr); sys.exit(1)
