// ════════════════════════════════════════════════════════════
// test_bot_1.js — PART 1/2  (run: cat test_bot_1.js test_bot_2.js > test_bot.js)
// ════════════════════════════════════════════════════════════
"use strict";
...
function log(lv,area,msg,data){
  if(LL[lv]<CL)return;
  const line=`[${new Date().toISOString()}][${lv.padEnd(5)}][${area}] ${msg}`;
  data!==undefined?console.log(line,typeof data==="object"?JSON.stringify(data):data):console.log(line);
}
...
const CFG={
  ...
  welcomeBody:[
    "Welcome, {user}!",
    "",
    "Please look at our <#1470989289708847105> first!",
    "If you need help, reach out to our team at any time.",
    "---",
    "Feel free to say hi, explore and meet new people! ♥️",
    "",
  ...
  kickBody:["Hey **{username}**,","","You were kicked from **{server}**.","---","**Reason:** (Check Dyno's message)","**Moderator:** {moderator}","---","You may rejoin if the link is still active."].join("\n"),
  muteTitle:"# 🔇 You were timed out in {server}",
  muteBody:["Hey **{username}**,","","You have been timed out in **{server}**.","---","**Reason:** (Check Dyno's message)","**Moderator:** {moderator}","**Until:** {until}","---","Follow the rules to avoid further action."].join("\n"),
};
...
function buildContainer(title,body,gifUrl,useAttachment=false){
  debug("CTR","Building",{tl:title?.length,bl:body?.length});
  const inner=[];
  inner.push({type:10,content:(title||"").trim().slice(0,4000)||"​"});
  ...
  secs.forEach((sec,i)=>{
    const t=sec.trim();
    if(t){splitText(t,3800).forEach(p=>{if(p.trim()){inner.push({type:10,content:p.trim()});hasBody=true;}});}
...
function buildEditor(s){
  ...
  return[
    ...
    {type:14,divider:true,spacing:1},
    {type:10,content:"-# 💡 Use `---` on its own line for separator · Set GIF to `-` for none · Body max 4000 chars"},
    {type:1,components:[
...
// ── PvZ Data ──────────────────────────────────────────────
const PLANTS={
  peashooter:  {name:"Peashooter",    cost:100,hp:300, emoji:"🌱",shootRate:3,dmg:20,projType:"pea"},
  sunflower:   {name:"Sunflower",     cost:50, hp:300, emoji:"🌻",sunRate:24,sunAmt:25},
  wallnut:     {name:"Wall-nut",      cost:50, hp:4000,emoji:"🥜"},
  cherrybomb:  {name:"Cherry Bomb",   cost:150,hp:300, emoji:"🍒",instant:true,aoe:true,aoeDmg:1800},
  potatomine:  {name:"Potato Mine",   cost:25, hp:300, emoji:"🥔",mine:true,mineDmg:1800,armDelay:15},
  snowpea:     {name:"Snow Pea",      cost:175,hp:300, emoji:"❄️",shootRate:3,dmg:20,projType:"snow",slow:true},
  chomper:     {name:"Chomper",       cost:150,hp:300, emoji:"👾",chomp:true,rechargeTicks:42},
  repeater:    {name:"Repeater",      cost:200,hp:300, emoji:"🌿",shootRate:3,dmg:20,projType:"pea",doubleShot:true},
  puffshroom:  {name:"Puff-shroom",   cost:0,  hp:300, emoji:"🍄",shootRate:4,dmg:20,projType:"pea",lifetime:120},
  sunshroom:   {name:"Sun-shroom",    cost:25, hp:300, emoji:"☀️",sunRate:24,sunAmt:15},
  fumeshroom:  {name:"Fume-shroom",   cost:75, hp:300, emoji:"💨",fume:true,fumeDmg:20,fumeRate:3},
  scaredy:     {name:"Scaredy-shroom",cost:25, hp:300, emoji:"😱",shootRate:3,dmg:20,projType:"pea",hideRange:3},
  iceshroom:   {name:"Ice-shroom",    cost:75, hp:300, emoji:"🧊",instant:true,freeze:true,freezeTicks:30},
  doomshroom:  {name:"Doom-shroom",   cost:125,hp:300, emoji:"💀",instant:true,globalAoe:true,aoeDmg:1800},
  squash:      {name:"Squash",        cost:50, hp:300, emoji:"🟩",instant:true,squash:true},
  threepeater: {name:"Threepeater",   cost:325,hp:300, emoji:"🌾",shootRate:3,dmg:20,projType:"pea",threeRow:true},
  torchwood:   {name:"Torchwood",     cost:175,hp:600, emoji:"🔥",torchwood:true},
  twinsunflower:{name:"Twin Sunflower",cost:150,hp:300,emoji:"🌼",sunRate:24,sunAmt:50},
  tallnut:     {name:"Tall-nut",      cost:125,hp:8000,emoji:"🧱"},
  cactus:      {name:"Cactus",        cost:125,hp:400, emoji:"🌵",shootRate:3,dmg:20,projType:"spike"},
};
const ZOMBIES={
...
function newGame(chId,uid,lv,uPl,uLv,lSt){
  info("PVZ","New game",{chId,uid,lv});
  const L=LEVELS[lv];
  return{channelId:chId,startedBy:uid,phase:"tutorial",level:lv,atmosphere:L.atm,wave:0,maxWaves:L.maxW,sun:L.sun,tick:0,
    grid:Array.from({length:5},()=>Array(9).fill(null)),zombies:[],projectiles:[],
    selectedPlant:null,plantPage:0,availablePlants:[...(START_PLANTS[lv]||START_PLANTS[0])],
    unlockedPlants:uPl||["peashooter","sunflower","wallnut"],unlockedLevels:uLv||[0],levelStars:lSt||{},
...
function newLvlSel(uid,uPl,uLv,lSt){
  return{phase:"level_select",startedBy:uid,unlockedPlants:uPl||["peashooter","sunflower","wallnut"],unlockedLevels:uLv||[0],levelStars:lSt||{},messageId:null,interactionToken:null,appId:null,channelId:null};
}

async function renderPvz(state){
  const tmp=path.join(os.tmpdir(),`pvz_${Date.now()}_${Math.random().toString(36).slice(2)}.png`);
  const py=path.join(__dirname,"pvz_render.py");
  debug("PVZ_R","Render",{phase:state.phase,tmp});
  if(!fs.existsSync(py))throw new Error("pvz_render.py not found at "+py);
  return new Promise((res,rej)=>{
    const proc=spawn("python3",[py,tmp]);
    let se="";
    proc.stderr.on("data",d=>{se+=d.toString();});
    proc.stdin.write(JSON.stringify(state));proc.stdin.end();
    proc.on("close",code=>{
      debug("PVZ_R","Python exit",code);
      if(code!==0){rej(new Error(`pvz_render.py exited ${code}: ${se}`));return;}
      try{const buf=fs.readFileSync(tmp);fs.unlink(tmp,()=>{});debug("PVZ_R","OK",{bytes:buf.length});res(buf);}catch(e){rej(e);}
    });
    proc.on("error",e=>{error("PVZ_R","Spawn error",e.message);rej(e);});
  });
}

function tick(gs){
  if(gs.phase!=="playing")return;gs.tick++;
  if(gs.tick%10===0){gs.sun+=25;debug("PVZ_T","Sun",gs.sun);}
  ...
  gs.projectiles.forEach(p=>{
    ...
    if(col>=0&&col<9&&gs.grid[p.row]?.[col]?.type==="torchwood"&&p.type==="pea"){p.type="fire";p.dmg=(p.dmg||20)*2;}
  ...
  gs.zombies.filter(z=>!z.dead).forEach(z=>{
    ...
    if(!z.frozen)z.x-=spd;if(z.slowTicks>0)z.slowTicks--;if(z.frozenTicks>0){z.frozenTicks--;if(z.frozenTicks===0)z.frozen=false;}
    const col=Math.max(0,Math.floor(z.x));const plant=gs.grid[z.row]?.[col];
    if(plant&&z.x<=col+0.8){plant.hp-=ZOMBIES[z.type].dmg;z.x=col+0.7;if(plant.hp<=0)gs.grid[z.row][col]=null;}
  });
  ...
  if(gs.zombies.some(z=>z.x<=-0.2)){warn("PVZ_T","Game over");gs.phase="game_over";return;}
  if(gs.waveQueue.length===0&&gs.zombies.length===0){
    if(gs.wave>=gs.maxWaves){
      info("PVZ_T","Victory",{lv:gs.level});gs.phase="victory";
    ...
    }else{gs.waveActive=false;info("PVZ_T","Wave cleared");}
  }
}

function spawnWave(gs){
  const def=LEVELS[gs.level].waves[gs.wave];if(!def){warn("PVZ","No wave def",{lv:gs.level,w:gs.wave});return;}
  info("PVZ",`Wave ${gs.wave+1}`);
...
function placePlant(gs,row){
  ...
  for(let col=0;col<9;col++){
    if(!gs.grid[row][col]){
      gs.sun-=pd.cost;gs.grid[row][col]={type:gs.selectedPlant,hp:pd.hp,maxHp:pd.hp,plantedAt:gs.tick,armed:false,armTick:null,chompCooldown:0};
      info("PVZ","Planted",{type:gs.selectedPlant,row,col,sun:gs.sun});
      if(pd.instant){
        if(pd.aoe||pd.globalAoe){const r=pd.globalAoe?99:1;gs.zombies.forEach(z=>{if(Math.abs(z.row-row)<=r&&Math.abs(z.x-col)<=r+1){z.hp-=pd.aoeDmg;if(z.hp<=0)z.dead=true;}});gs.grid[row][col]=null;}
...
function buildPvzUI(gs){
  if(gs.phase==="level_select"){
    const btns=LEVELS.map((l,i)=>({type:2,style:gs.unlockedLevels.includes(i)?1:2,label:gs.unlockedLevels.includes(i)?`${i+1}. ${l.name.split("–")[0].trim()}`:`🔒 Lv${i+1}`,custom_id:`pvz_level_${i}`,disabled:!gs.unlockedLevels.includes(i)}));
    return[{type:17,components:[{type:10,content:"## 🌻  Plants vs. Zombies\nSelect a level!"},{type:14,divider:true,spacing:1},{type:10,content:LEVELS.map((l,i)=>{const lo=!gs.unlockedLevels.includes(i);const s=gs.levelStars[i]?"⭐".repeat(gs.levelStars[i]):"☆☆☆";return`${lo?"🔒":"🌱"} **Lv${i+1}** ${l.name} ${lo?"*(locked)*":`[${s}]`}`;}).join("\n")}]},{type:1,components:btns.slice(0,5)}];
  }
  if(gs.phase==="tutorial"){
    const steps=["**☀️ Sun** = currency. Sunflowers produce it!","**🌱 Select plant** then press **Lane** button.","**🧟 Zombies** walk right→left. Stop them!","**⚡ Next Turn** = plants shoot & zombies move.","Between waves — plan before next wave!"];
    return[{type:17,components:[{type:10,content:`## 📖 Tutorial (${gs.tutorialStep+1}/5)`},{type:14,divider:true,spacing:1},{type:10,content:steps[gs.tutorialStep]||steps[0]}]},{type:1,components:[{type:2,style:1,label:gs.tutorialStep<4?"Next ▶":"Start! 🌻",custom_id:"pvz_tut_next"},{type:2,style:2,label:"Skip",custom_id:"pvz_tut_skip"}]}];
  }
  const plants=gs.availablePlants,page=gs.plantPage||0,ps=5;
  const pBtns=plants.slice(page*ps,(page+1)*ps).map(pt=>{const pd=PLANTS[pt];const ok=gs.sun>=pd.cost;return{type:2,style:gs.selectedPlant===pt?3:(ok?1:2),label:`${pd.emoji} ${pd.name} ☀️${pd.cost}`,custom_id:`pvz_select_${pt}`,disabled:!ok&&gs.selectedPlant!==pt};});
  ...
  const aBtns=[
    ...
    btwn?{type:2,style:3,label:"▶ Next Wave",custom_id:"pvz_nextwave"}:{type:2,style:2,label:`☀️ ${gs.sun}`,custom_id:"pvz_suninfo",disabled:true},
    maxP>0?{type:2,style:2,label:`Plants p.${page+1}/${maxP+1}`,custom_id:"pvz_plantpage"}:{type:2,style:2,label:`☀️ ${gs.sun}`,custom_id:"pvz_sun2",disabled:true},
    {type:2,style:4,label:"❌ Quit",custom_id:"pvz_quit"},
  ];
  let inf=`**Wave ${gs.wave}/${gs.maxWaves}** · ☀️ ${gs.sun} · ${gs.zombies.length} zombie(s)`;
  if(btwn)inf+="\n**Wave cleared!** Start next when ready.";
  if(gs.shovelMode)inf+="\n⛏️ **Shovel** — pick Lane to remove plant.";
  if(gs.selectedPlant)inf+=`\n🌱 **${PLANTS[gs.selectedPlant].name}** — pick a Lane.`;
  ...
  if(gs.phase==="game_over")comps.push({type:1,components:[{type:2,style:1,label:"🔄 Restart",custom_id:"pvz_restart"},{type:2,style:2,label:"🏠 Level Select",custom_id:"pvz_lvlselect"}]});
  if(gs.phase==="victory"){const hn=gs.level+1<LEVELS.length;comps.push({type:1,components:[hn?{type:2,style:3,label:"▶ Next Level",custom_id:"pvz_nextlevel"}:{type:2,style:1,label:"🏆 All done!",custom_id:"pvz_noop_win",disabled:true},{type:2,style:2,label:"🏠 Level Select",custom_id:"pvz_lvlselect"}]});}
  return comps;
}

async function updateGame(gs,interaction,isNew=false){
  info("PVZ_M",`Update (new=${isNew})`,{phase:gs.phase});
  const buf=await renderPvz(gs);const att=new AttachmentBuilder(buf,{name:"game.png"});const comps=buildPvzUI(gs);
  if(isNew){const msg=await interaction.reply({files:[att],components:comps,flags:MessageFlags.IsComponentsV2,fetchReply:true});gs.messageId=msg.id;gs.interactionToken=interaction.token;gs.appId=interaction.applicationId;info("PVZ_M","Sent",msg.id);}
  else{await interaction.update({files:[att],components:comps,flags:MessageFlags.IsComponentsV2});debug("PVZ_M","Updated");}
}
async function sendLvlSel(interaction,gs){
  info("PVZ_M","LvlSel");const buf=await renderPvz(gs);const att=new AttachmentBuilder(buf,{name:"game.png"});
  ...
  pvzGames.set(interaction.channelId,gs);info("PVZ_M","Sent",msg.id);
}
// ─── END PART 1 ─── append test_bot_2.js below ───────────
// ════════════════════════════════════════════════════════════
...
client.once(Events.ClientReady,async(c)=>{
  ...
  info("BOT","pvz_render.py exists?",fs.existsSync(path.join(__dirname,"pvz_render.py")));
  info("BOT","video.gif exists?",    fs.existsSync(CFG.gifPath));
  info("BOT","collab.js exists?",    fs.existsSync(path.join(__dirname,"collab.js")));
  ...
  const commands=[
    ...
    new SlashCommandBuilder().setName("test")
      ...
      .addStringOption(o=>o.setName("type").setDescription("Message type").setRequired(true)
        .addChoices({name:"Welcome",value:"welcome"},{name:"Ban",value:"ban"},{name:"Kick",value:"kick"},{name:"Mute",value:"mute"}))
      .toJSON(),
    new SlashCommandBuilder().setName("pvz")
      .setDescription("Start a Plants vs. Zombies game (everyone can see it!)").toJSON(),
    new SlashCommandBuilder().setName("collab")
      .setDescription("Create a collaboration project").toJSON(),
  ];

  const rest=new REST({version:"10"}).setToken(CFG.token);
  const appId=CFG.clientId||c.user.id;
  info("BOT","Registering commands for",appId);
  try{
...
client.on("interactionCreate",async(interaction)=>{
  ...
  if(interaction.isChatInputCommand()&&interaction.commandName==="test"){
    ...
    try{
      ...
      await interaction.reply({components:[{type:10,content:"❌  Could not send DM. Enable DMs from server members."}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});
    }
    return;
  }
  ...
  if(interaction.isChatInputCommand()&&interaction.commandName==="pvz"){
    ...
    if(ex&&ex.phase!=="game_over"&&ex.phase!=="victory"){
      warn("PVZ","Game already active",interaction.channelId);
      await interaction.reply({components:[{type:10,content:"❌  A game is already running! Finish it or press ❌ Quit."}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});
      return;
    }
    ...
    try{await sendLvlSel(interaction,gs);}
      ...
      error("PVZ","Start failed",e.message);
      await interaction.reply({components:[{type:10,content:`❌  Failed to start game: \`${e.message}\`\nMake sure \`pillow\` is installed: \`pip install pillow\``}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});
    }
    return;
  }
  ...
  if(interaction.isButton()){
    ...
    // Collab buttons
    if(collab.isCollabButton(cid)){
      await collab.handleButton(interaction);
      return;
    }
    ...
    if(["edit_title","edit_body","edit_gif","preview_msg","send_msg","cancel_composer"].includes(cid)){
      ...
      if(!s){
        ...
        await interaction.reply({components:[{type:10,content:"❌  Session expired. Run `/send` again."}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});
        return;
      }
      if(cid==="edit_title"){
        await interaction.showModal(new ModalBuilder().setCustomId("modal_title").setTitle("Edit Title")
          .addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("input").setLabel("Title (markdown OK, e.g. # Heading)").setStyle(TextInputStyle.Short).setValue(s.title).setRequired(true))));
        return;
      }
      if(cid==="edit_body"){
        await interaction.showModal(new ModalBuilder().setCustomId("modal_body").setTitle("Edit Body")
          .addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("input").setLabel("Body (use --- alone on a line for divider)").setStyle(TextInputStyle.Paragraph).setValue(s.body.slice(0,4000)).setRequired(true).setMaxLength(4000))));
        return;
      }
      if(cid==="edit_gif"){
        await interaction.showModal(new ModalBuilder().setCustomId("modal_gif").setTitle("Edit GIF URL")
          .addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("input").setLabel("GIF URL  (type - for no GIF)").setStyle(TextInputStyle.Short).setValue(s.gifUrl||"-").setRequired(true))));
        return;
      }
      if(cid==="preview_msg"){
        info("COMP","Preview",interaction.user.tag);
        await interaction.reply({components:[{type:10,content:"-# 👁  Preview — only you can see this"},{type:14,divider:true,spacing:1},...buildContainer(s.title,s.body,s.gifUrl)],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});
        return;
      }
      if(cid==="send_msg"){
    ...
    if(cid.startsWith("pvz_")){
      ...
      if(!gs){
        warn("PVZ","No active game",interaction.channelId);
        await interaction.reply({components:[{type:10,content:"❌  No active game. Run `/pvz`."}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});
        return;
      }
      ...
      if(cid.startsWith("pvz_level_")){
        ...
        info("PVZ","Start level",{li,user:interaction.user.tag});
      ...
      if(cid==="pvz_tut_next"){
        ...
        else{gs.phase="playing";gs.tutorialDone=true;spawnWave(gs);info("PVZ","Tutorial done");}
      ...
      if(cid.startsWith("pvz_select_")){
        ...
        if(gs.selectedPlant===pt){gs.selectedPlant=null;debug("PVZ","Deselect");}
        else{
          const pd=PLANTS[pt];
          if(gs.sun<pd.cost){await interaction.reply({components:[{type:10,content:`☀️  Need **${pd.cost}** sun, have **${gs.sun}**.`}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});return;}
      ...
      if(cid.startsWith("pvz_lane_")){
        ...
        if(gs.shovelMode){
          let removed=false;
          for(let col=8;col>=0;col--){if(gs.grid[row][col]){gs.grid[row][col]=null;gs.shovelMode=false;removed=true;info("PVZ","Shoveled",{row,col});break;}}
      ...
      if(cid==="pvz_tick"){
        debug("PVZ","Tick",{t:gs.tick,user:interaction.user.tag});
        for(let i=0;i<3;i++)tick(gs);
        try{await updateGame(gs,interaction);}catch(e){error("PVZ","Render error",e.message);}
        return;
      }
      if(cid==="pvz_shovel"){
        gs.shovelMode=!gs.shovelMode;gs.selectedPlant=null;
        info("PVZ","Shovel toggled",{shovel:gs.shovelMode,user:interaction.user.tag});
        try{await updateGame(gs,interaction);}catch(e){error("PVZ","Render error",e.message);}
        return;
      }
      if(cid==="pvz_nextwave"){
        if(gs.waveActive||gs.zombies.length>0){await interaction.reply({components:[{type:10,content:"⚔️  Finish current wave first!"}],flags:MessageFlags.IsComponentsV2|MessageFlags.Ephemeral});return;}
        info("PVZ","Next wave",{w:gs.wave+1,user:interaction.user.tag});
        spawnWave(gs);
        try{await updateGame(gs,interaction);}catch(e){error("PVZ","Render error",e.message);}
        return;
      }
      if(cid==="pvz_plantpage"){
        const mp=Math.ceil(gs.availablePlants.length/5)-1;gs.plantPage=((gs.plantPage||0)+1)%(mp+1);
        debug("PVZ","Plant page",gs.plantPage);
        try{await updateGame(gs,interaction);}catch(e){error("PVZ","Render error",e.message);}
        return;
      }
      if(cid==="pvz_restart"){
        info("PVZ","Restart",{lv:gs.level,user:interaction.user.tag});
      ...
      if(cid==="pvz_lvlselect"){
        ...
        try{
          const buf=await renderPvz(ls);const att=new AttachmentBuilder(buf,{name:"game.png"});
      ...
      if(cid==="pvz_nextlevel"){
        ...
        info("PVZ","Next level",{ni,user:interaction.user.tag});
  ...
  if(interaction.isModalSubmit()){
    ...
    // Collab modals
    if(collab.isCollabModal(cid)){
      await collab.handleModal(interaction);
      return;
    }
...
client.on("guildBanAdd",async(ban)=>{
  ...
  try{
    ...
    if(e&&e.targetId===user.id){mod=`<@${e.executorId}>`;reason=e.reason??reason;}
    debug("MOD","Audit",{mod,reason});
  ...
  }catch(e){warn("MOD","Ban DM failed",{user:user.tag,err:e.message});}
});

client.on("guildMemberUpdate",async(old,nw)=>{
  const ot=old.communicationDisabledUntilTimestamp??null;
  const nt=nw.communicationDisabledUntilTimestamp??null;
  if(ot||!nt)return;
  info("MOD","Timeout",nw.user.tag);
  let mod="Server moderation",reason="No reason provided";
  try{
    const logs=await nw.guild.fetchAuditLogs({limit:5});
    const e=logs.entries.find(e=>e.targetId===nw.id);
    if(e){mod=`<@${e.executorId}>`;reason=e.reason??reason;}
  }catch(e){warn("MOD","Audit logs unavailable",e.message);}
  const until=new Date(nt).toUTCString();
  ...
  }catch(e){warn("MOD","Mute DM failed",{user:nw.user.tag,err:e.message});}
});
...
client.on("guildMemberRemove",async(member)=>{
  try{
    ...
    }catch(e2){warn("MOD","Kick DM failed",{user:member.user.tag,err:e2.message});}
...
client.on("guildMemberAdd",async(member)=>{
  ...
  try{
    await channel.send({files:[att],components:buildContainer(fill(CFG.welcomeTitle,data),fill(CFG.welcomeBody,data),null,true),flags:MessageFlags.IsComponentsV2});
...
process.on("unhandledRejection",e=>error("PROC","Unhandled rejection",e?.message||e));
process.on("uncaughtException",  e=>error("PROC","Uncaught exception", e?.message||e));
info("BOT","Starting...");
