...
// Build: cat test_bot_1.js test_bot_2.js > bot.js  then node bot.js
// ═══════════════════════════════════════════════════════════════════
"use strict";
...
function log(lv,area,msg,data){
  if(LL[lv]<CL)return;
  const line=`[${new Date().toISOString()}] ${EMOJIS[lv]||""} [${lv.padEnd(5)}][${area}] ${msg}`;
...
const CFG={
  ...
  welcomeBody: [
    ...
    "---",
    "Feel free to introduce yourself, explore the channels, and meet new people! ♥️",
    "",
  ...
  banBody:   ["Hello **{username}**,","","You have been permanently banned from **{server}**.","---","**Reason:** (Check Dyno's message for details)","**Moderator:** {moderator}","---","To appeal, visit: https://appeal.gg/TVku2Yhtgw"].join("\n"),
...
// ── Admin check (role OR Administrator permission) ────────────────
...
// Build a Components V2 container (type 17 = gray box)
// Returns array with one container element.
// If text is very long, caller should detect and split at a higher level.
function buildContainer(title, body, gifUrl, useAttachment=false) {
  debug("CTR","Building",{tl:title?.length,bl:body?.length});
  const inner=[];

  // Title
  const safeTitle=(title||"").trim().slice(0,1800)||" ";
  inner.push({type:10, content:safeTitle});
  inner.push({type:14, divider:true, spacing:1});

  // Body: normalize line endings then split on --- separators
  ...
  secs.forEach((sec,i)=>{
    const t=sec.trim();
    if(t){
      // Split long sections into chunks of ≤900 chars
      const parts=splitText(t,900);
      for(const p of parts){
        if(p.trim()){inner.push({type:10,content:p.trim()});hasBody=true;}
      }
    }
  ...
  debug("CTR",`Container: ${inner.length} inner components`);
  return [{type:17,components:inner}];
}

// Build the /send composer editor UI
...
function newGame(chId,uid,lv,uPl,uLv,lSt){
  const LV=LEVELS[lv];
  return{channelId:chId,startedBy:uid,phase:"tutorial",level:lv,atmosphere:LV.atm,wave:0,maxWaves:LV.maxW,sun:LV.sun,tick:0,
...
function newLvlSel(uid,uPl,uLv,lSt){
  return{phase:"level_select",startedBy:uid,unlockedPlants:uPl||["peashooter","sunflower","wallnut"],
    unlockedLevels:uLv||[0],levelStars:lSt||{},messageId:null,interactionToken:null,appId:null,channelId:null};
}
...
async function renderPvz(state){
  ...
  return new Promise((res,rej)=>{
    ...
    proc.on("close",code=>{
      if(code!==0){rej(new Error(`pvz_render.py exited ${code}: ${se.trim()}`));return;}
      try{const buf=fs.readFileSync(tmp);fs.unlink(tmp,()=>{});res(buf);}catch(e){rej(e);}
    });
...
function gameTick(gs){
  ...
  gs.zombies.filter(z=>!z.dead).forEach(z=>{
    ...
    if(!z.frozen)z.x-=spd; if(z.slowTicks>0)z.slowTicks--;
    if(z.frozenTicks>0){z.frozenTicks--;if(z.frozenTicks===0)z.frozen=false;}
    const c2=Math.max(0,Math.floor(z.x)); const plant=gs.grid[z.row]?.[c2];
...
function placePlant(gs,row){
  ...
  for(let col=0;col<9;col++){
    if(!gs.grid[row][col]){
      ...
      gs.selectedPlant=null; gs.zombies=gs.zombies.filter(z=>!z.dead);
      return null;
    }
  }
  return "Lane is full.";
...
// Build Discord Components V2 UI for PvZ game
function buildPvzUI(gs){
  if(gs.phase==="level_select"){
    const btns=LEVELS.map((lv,i)=>({type:2,style:gs.unlockedLevels.includes(i)?1:2,
      label:gs.unlockedLevels.includes(i)?`${i+1}. ${lv.name.split("–")[0].trim()}`:`🔒 Level ${i+1}`,
      custom_id:`pvz_level_${i}`,disabled:!gs.unlockedLevels.includes(i)}));
    return[
      {type:17,components:[
        {type:10,content:"## 🌻  Plants vs. Zombies\nSelect a level to begin!"},
        {type:14,divider:true,spacing:1},
        {type:10,content:LEVELS.map((lv,i)=>{const lo=!gs.unlockedLevels.includes(i);const st=gs.levelStars[i]?"⭐".repeat(gs.levelStars[i]):"☆☆☆";return`${lo?"🔒":"🌱"} **Level ${i+1}** – ${lv.name}  ${lo?"*(locked)*":`[${st}]`}`;}).join("\n")},
      ]},
      {type:1,components:btns},
    ];
  }
  if(gs.phase==="tutorial"){
    const steps=["**☀️ Sun** is your currency — Sunflowers produce it automatically. Collect sun to buy and place plants!","**🌱 Plant Selection** — Press a plant button below to select it, then press a Lane button to place it on the field.","**🧟 Zombies** enter from the right side and walk left. Do not let them reach the left edge!","**⚡ Next Turn** — Press this button to advance time. Plants will shoot and zombies will move each turn.","**Between Waves** — When a wave is cleared, plan your defenses before pressing Next Wave to continue."];
    return[
      {type:17,components:[
        ...
        {type:14,divider:true,spacing:1},
        {type:10,content:"-# The image above highlights the relevant UI element for this step."},
      ]},
      {type:1,components:[
  ...
  const pageItems=plants.slice(page*ps,(page+1)*ps);

  // Plant buttons — no padding with empty labels
  const pBtns=pageItems.map(pt=>{const pd=PLANTS[pt];const ok=gs.sun>=pd.cost;return{type:2,style:gs.selectedPlant===pt?3:(ok?1:2),label:`${pd.emoji} ${pd.name} ☀️${pd.cost}`,custom_id:`pvz_select_${pt}`,disabled:!ok&&gs.selectedPlant!==pt};});
  // Pad only with visible label
  ...
  const aBtns=[
    ...
    btwn?{type:2,style:3,label:"▶ Next Wave",custom_id:"pvz_nextwave"}:{type:2,style:2,label:`☀️ ${gs.sun} sun`,custom_id:"pvz_suninfo",disabled:true},
    maxP>0?{type:2,style:2,label:`Plants ${page+1}/${maxP+1}`,custom_id:"pvz_plantpage"}:{type:2,style:2,label:`☀️ ${gs.sun} sun`,custom_id:"pvz_sun2",disabled:true},
    {type:2,style:4,label:"❌ Quit",custom_id:"pvz_quit"},
  ];
  let inf=`**Wave ${gs.wave}/${gs.maxWaves}**  ·  ☀️ ${gs.sun} sun  ·  ${gs.zombies.length} zombie(s) on field`;
  if(btwn) inf+="\n**Wave cleared!** Start the next wave when ready.";
  if(gs.shovelMode) inf+="\n⛏️ **Shovel mode** — select a Lane to remove the rightmost plant.";
  if(gs.selectedPlant) inf+=`\n🌱 **${PLANTS[gs.selectedPlant].name}** selected — press a Lane button to place.`;
  ...
  if(gs.phase==="victory"){const hn=gs.level+1<LEVELS.length;comps.push({type:1,components:[hn?{type:2,style:3,label:"▶ Next Level",custom_id:"pvz_nextlevel"}:{type:2,style:1,label:"🏆 All Levels Completed!",custom_id:"pvz_noop_win",disabled:true},{type:2,style:2,label:"🏠 Level Select",custom_id:"pvz_lvlselect"}]});}
  return comps;
}

async function updateGame(gs,interaction,isNew=false){
  info("PVZ_M",`Update (new=${isNew})`,{phase:gs.phase,wave:gs.wave});
  const buf=await renderPvz(gs);
...
async function sendLvlSel(interaction,gs){
  info("PVZ_M","Sending level select");
  const buf=await renderPvz(gs);
  const att=new AttachmentBuilder(buf,{name:"game.png"});
...
client.once(Events.ClientReady, async (c) => {
  ...
  info("BOT", "Application ID", c.user.id);
  info("BOT", "pvz_render.py found?", fs.existsSync(path.join(__dirname, "pvz_render.py")));
  info("BOT", "video.gif found?",     fs.existsSync(CFG.gifPath));
  info("BOT", "collab.js found?",     fs.existsSync(path.join(__dirname, "collab.js")));

  // Admin-only permission bit (role enforced at handler level)
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      ...
      .setDefaultMemberPermissions(adminPerm)
      .addStringOption(o =>
        o.setName("type").setDescription("Message type to preview").setRequired(true)
          .addChoices(
    ...
    new SlashCommandBuilder()
      .setName("pvz")
      .setDescription("Start a Plants vs. Zombies game — visible to everyone!")
      .toJSON(),
    new SlashCommandBuilder()
      .setName("collab")
      .setDescription("Create a new collaboration project")
      .toJSON(),
  ];
  ...
  try {
    ...
    error("BOT", "Failed to register commands", e.message);
  }
});
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === "send") {
    if (!isAdminUser(interaction.member)) {
      await interaction.reply({
        components: [{ type: 10, content: "❌  You need the admin role to use this command." }],
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === "test") {
    ...
    if (type === "welcome") {
      files      = [new AttachmentBuilder(CFG.gifPath, { name: "video.gif" })];
      components = buildContainer(fill(CFG.welcomeTitle, ph), fill(CFG.welcomeBody, ph), null, true);
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === "pvz") {
    ...
    if (ex && ex.phase !== "game_over" && ex.phase !== "victory") {
      ...
      await interaction.reply({
        components: [{ type: 10, content: "❌  A game is already running in this channel. Finish it or press **❌ Quit**." }],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
      return;
    }
    ...
    try { await sendLvlSel(interaction, gs); }
      ...
      await interaction.reply({
        components: [{ type: 10, content: `❌  Failed to start game: \`${e.message}\`\nEnsure \`pillow\` is installed: \`pip install pillow\`` }],
        flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      });
    }
    return;
  ...
  if (interaction.isButton()) {
    ...
    if (["edit_title","edit_body","edit_gif","preview_msg","send_msg","cancel_composer"].includes(cid)) {
      ...
      if (!s) {
        warn("COMP", "No session found", interaction.user.tag);
        await interaction.reply({
          components: [{ type: 10, content: "❌  Session expired. Please run `/send` again." }],
          flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
        });
        return;
      }
      ...
      if (cid === "edit_body") {
        await interaction.showModal(
          new ModalBuilder().setCustomId("modal_body").setTitle("Edit Body")
            .addComponents(new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("input")
                .setLabel("Body text  (use --- alone on a line for divider)")
      ...
      if (cid === "edit_gif") {
        await interaction.showModal(
          new ModalBuilder().setCustomId("modal_gif").setTitle("Edit GIF URL")
            .addComponents(new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("input")
                .setLabel("Direct GIF URL  (type - for no GIF)")
      ...
      if (cid === "preview_msg") {
        info("COMP", "Preview requested", interaction.user.tag);
        const preview = buildContainer(s.title, s.body, s.gifUrl);
        await interaction.reply({
          components: [
            { type: 10, content: "-# 👁  Preview — only visible to you" },
            { type: 14, divider: true, spacing: 1 },
            ...preview,
          ],
          flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
      ...
      if (cid === "send_msg") {
        ...
        if (!ch) {
          await interaction.reply({
            components: [{ type: 10, content: "❌  Target channel not found." }],
            flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
          });
          return;
        }
        info("COMP", "Sending message", { channel: s.channelId, user: interaction.user.tag });

        // Build container and check if it needs to be split
        const norm = (s.body || "").replace(/\r\n/g, "\n").replace(/\r/g, "\n");
        const totalLen = s.title.length + norm.length;
        const SOFT_LIMIT = 3500; // stay comfortably under Discord's 4096 limit per component

        if (totalLen > SOFT_LIMIT) {
          // Split: first message = title + first half of body, second = rest
          const midPoint  = Math.floor(norm.length / 2);
          // Find nearest newline to midpoint
          let splitIdx = norm.lastIndexOf("\n", midPoint);
          if (splitIdx < 0) splitIdx = midPoint;

          const bodyPart1 = norm.slice(0, splitIdx).trim();
          const bodyPart2 = norm.slice(splitIdx).trim();

          warn("COMP", "Message too long, splitting into 2", { totalLen, splitAt: splitIdx });

          await ch.send({
            components: buildContainer(s.title, bodyPart1, null),
            flags: MessageFlags.IsComponentsV2,
          });
          await ch.send({
            components: buildContainer("*(continued)*", bodyPart2, s.gifUrl),
            flags: MessageFlags.IsComponentsV2,
          });
          ...
          await interaction.update({
            components: [{
              type: 10,
              content: "✅  Message was too long and was sent as **2 separate messages**.",
            }],
            flags: MessageFlags.IsComponentsV2,
          });
        } else {
          await ch.send({
            components: buildContainer(s.title, s.body, s.gifUrl),
            flags: MessageFlags.IsComponentsV2,
          });
          sessions.delete(interaction.user.id);
          await interaction.update({
    ...
    if (cid.startsWith("pvz_")) {
      ...
      if (!gs) {
        ...
        await interaction.reply({
          components: [{ type: 10, content: "❌  No active game found. Run `/pvz` to start one." }],
          flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
        });
        return;
      }
      ...
      if (cid.startsWith("pvz_level_")) {
        ...
        if (!gs.unlockedLevels.includes(li)) {
          await interaction.reply({
            components: [{ type: 10, content: "🔒  This level is locked. Complete the previous level first." }],
            flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
          });
          return;
        }
      ...
      if (cid === "pvz_tut_next") {
        if (gs.tutorialStep < 4) {
          gs.tutorialStep++;
          debug("PVZ", "Tutorial step", gs.tutorialStep);
        } else {
          gs.phase       = "playing";
          gs.tutorialDone = true;
          spawnWave(gs);
          info("PVZ", "Tutorial complete — game starting");
        }
      ...
      if (cid === "pvz_tut_skip") {
        ...
        gs.tutorialDone = true;
        spawnWave(gs);
        info("PVZ", "Tutorial skipped by", interaction.user.tag);
        try { await updateGame(gs, interaction); } catch (e) { error("PVZ", "Render error", e.message); }
        return;
      }
      ...
      if (cid.startsWith("pvz_select_")) {
        ...
        if (gs.selectedPlant === pt) {
          gs.selectedPlant = null;
          debug("PVZ", "Plant deselected");
        } else {
          const pd = PLANTS[pt];
          if (gs.sun < pd.cost) {
            await interaction.reply({
          ...
          gs.selectedPlant = pt;
          gs.shovelMode    = false;
          info("PVZ", "Plant selected", { plant: pt, user: interaction.user.tag });
        }
        try { await updateGame(gs, interaction); } catch (e) { error("PVZ", "Render error", e.message); }
        return;
      }
      ...
      if (cid.startsWith("pvz_lane_")) {
        ...
        info("PVZ", "Lane button", { row, shovel: gs.shovelMode, plant: gs.selectedPlant, user: interaction.user.tag });
        if (gs.shovelMode) {
          let removed = false;
          for (let col = 8; col >= 0; col--) {
            if (gs.grid[row][col]) {
              ...
              gs.shovelMode     = false;
              removed           = true;
              info("PVZ", "Plant shoveled", { row, col });
              break;
            }
          }
          if (!removed) {
            await interaction.reply({
              components: [{ type: 10, content: "⛏️  No plant found in that lane." }],
              flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
            });
            return;
          }
          ...
          if (!gs.selectedPlant) {
            await interaction.reply({
              components: [{ type: 10, content: "🌱  Please select a plant first." }],
              flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
            });
            return;
          }
      ...
      if (cid === "pvz_nextwave") {
        if (gs.waveActive || gs.zombies.length > 0) {
          await interaction.reply({
            components: [{ type: 10, content: "⚔️  Finish the current wave before starting the next." }],
            flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
          });
          return;
        }
        info("PVZ", "Starting next wave", { wave: gs.wave + 1, user: interaction.user.tag });
        spawnWave(gs);
        try { await updateGame(gs, interaction); } catch (e) { error("PVZ", "Render error", e.message); }
        return;
      }
      ...
      if (cid === "pvz_plantpage") {
        ...
        debug("PVZ", "Plant page flipped", gs.plantPage);
        try { await updateGame(gs, interaction); } catch (e) { error("PVZ", "Render error", e.message); }
        return;
      }

      if (cid === "pvz_restart") {
        info("PVZ", "Restarting level", { level: gs.level, user: interaction.user.tag });
        const ng = newGame(interaction.channelId, interaction.user.id, gs.level, gs.unlockedPlants, gs.unlockedLevels, gs.levelStars);
        ng.phase       = "playing";
        ng.tutorialDone = true;
        spawnWave(ng);
        pvzGames.set(interaction.channelId, ng);
      ...
      if (cid === "pvz_lvlselect") {
        info("PVZ", "Returning to level select", interaction.user.tag);
        ...
        try {
          const buf = await renderPvz(ls);
          const att = new AttachmentBuilder(buf, { name: "game.png" });
          await interaction.update({
            files:      [att],
            components: buildPvzUI(ls),
            flags:      MessageFlags.IsComponentsV2,
          });
      ...
      if (cid === "pvz_nextlevel") {
        ...
        if (ni >= LEVELS.length) {
          await interaction.reply({
            components: [{ type: 10, content: "🏆  You have completed all available levels!" }],
            flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
          });
          return;
        }
        info("PVZ", "Advancing to next level", { level: ni, user: interaction.user.tag });
      ...
      if (cid === "pvz_quit") {
        info("PVZ", "Game quit by", interaction.user.tag);
        pvzGames.delete(interaction.channelId);
        await interaction.update({
          files:      [],
          components: [{
            type: 17,
            components: [{ type: 10, content: "## 🌻  Game Ended\nRun `/pvz` again whenever you want to play!" }],
          }],
          flags: MessageFlags.IsComponentsV2,
        });
        return;
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (cid === "modal_gif")   { s.gifUrl = val; info("COMP", "GIF URL updated", val); }
    sessions.set(interaction.user.id, s);
...
client.on("guildBanAdd", async (ban) => {
  ...
  info("MOD", "Ban detected", { user: user.tag, guild: guild.name });
  ...
  try {
    ...
    if (entry && entry.targetId === user.id) {
      mod    = `<@${entry.executorId}>`;
      reason = entry.reason ?? reason;
    }
    debug("MOD", "Audit log resolved", { mod, reason });
  } catch (e) { warn("MOD", "Could not fetch audit logs", e.message); }

  const d = { user: `<@${user.id}>`, username: user.username, server: guild.name, moderator: mod, reason };
  try {
    await user.send({
      components: buildContainer(fill(CFG.banTitle, d), fill(CFG.banBody, d), null),
      flags: MessageFlags.IsComponentsV2,
    });
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
  ...
  if (oldT || !newT) return;
  info("MOD", "Timeout applied", newMember.user.tag);
  let mod = "Server moderation", reason = "No reason provided";
  try {
  ...
  const d = { user: `<@${newMember.id}>`, username: newMember.user.username, server: newMember.guild.name, moderator: mod, reason, until };
  try {
    await newMember.send({
      components: buildContainer(fill(CFG.muteTitle, d), fill(CFG.muteBody, d), null),
      flags: MessageFlags.IsComponentsV2,
    });
    info("MOD", "Mute DM sent", newMember.user.tag);
  } catch (e) { warn("MOD", "Could not DM muted user", { user: newMember.user.tag, err: e.message }); }
});
...
client.on("guildMemberRemove", async (member) => {
  try {
    ...
    if (!entry) return;
    info("MOD", "Kick detected", member.user.tag);
    const d = {
      user:      `<@${member.id}>`,
    ...
    try {
      await member.send({
        components: buildContainer(fill(CFG.kickTitle, d), fill(CFG.kickBody, d), null),
        flags: MessageFlags.IsComponentsV2,
      });
      info("MOD", "Kick DM sent", member.user.tag);
    } catch (e) { warn("MOD", "Could not DM kicked user", { user: member.user.tag, err: e.message }); }
  } catch (e) { warn("MOD", "Could not fetch audit logs for kick detection", e.message); }
});
...
client.on("guildMemberAdd", async (member) => {
  ...
  if (!channel) { error("WELCOME", "Welcome channel not found", CFG.welcomeChannelId); return; }
  const data = {
    user:     `<@${member.id}>`,
    username: member.user.username,
    server:   member.guild.name,
  };
  const attachment = new AttachmentBuilder(CFG.gifPath, { name: "video.gif" });
  try {
    await channel.send({
      files:      [attachment],
    ...
    info("WELCOME", "Welcome message sent", member.user.tag);
  } catch (e) {
    error("WELCOME", "Failed to send welcome message", e.message);
  }
});

// ── Start ─────────────────────────────────────────────────────────
if (!CFG.token) {
  error("BOT", "DISCORD_TOKEN is not set in environment variables.");
  process.exit(1);
}
